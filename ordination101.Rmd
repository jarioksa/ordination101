...
- In unconstrained ordination we first find the ordination as well as
  possible and then see how it can be explained
...
- In unconstrained ordination we found the *linear combination* of
  species that best explains all species

- Now we find the *linear combination* of **constraints** that best
  explains all species
...
ylim <- range(bott0)
for(i in 1:3) {
   plot(rda1, bott0[,i], ylim=ylim, xlab=koef, ylab=colnames(bott0)[i], type="n", cex.lab=1.5)
...
ylim <- range(fv)
for(i in 1:3) {
   plot(rda1, fv[,i], ylim=ylim, xlab=koef, ylab=colnames(fv)[i], type="n", cex.lab=1.5)
