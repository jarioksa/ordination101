...
data('varespec', 'varechem', 'mite', 'mite.env', 'dune', 'dune.env', 'BCI', 'BCI.env')
...
- SUs within clusters can be permuted freely, oa as unidimensional
  series or as spatial grids which can be mirrorred

- Permutations can be constant or independent within clusters
...
- If we have a strong advance theory we inspect, we keep and analyse
  all variabels of interest &mdash; even those that prove to be
  insignficant
...
- Too many regressors is overfitting: Absolute limit is taht the
  number of constraints equals the number of observations, and we have
  a complete fit with no residual error

- Two constraints can allow the return of the curve if these are
  similar to unconstrained axes or have quadratic relation
