...
```{r bottpredict, fig.width=8, fig.height=5.4}
...
```{r factorshow, fig.width=8, fig.height=5.4}
...
```{r rdamethod2a, fig.width=8, fig.height=5.4}
par(mar=c(4,4,0,1)+.1)
palette(viridis(8))
...
plot(mrda, dis=c("wa","bp"), type="n")
points(mrda, dis="wa", pch=21, bg=8, col=1)
points(mrda, dis="lc", pch=21, bg=4, col=1)
text(mrda, dis="bp", col=1, lwd=2)
Alfit <- ordisurf(mrda ~ Al, varechem, plot=FALSE, knots=1, display="lc")
Pfit <- ordisurf(mrda ~ P, varechem, plot=FALSE, knots=1,display="lc")
xy <- scores(mrda, dis="wa", choices=1:2)
...
dim(fit) <- c(31,31)
contour(xn1, xn2, fit, add=TRUE, col=5)
legend("bottomleft", c("LC","WA"), pch=21, pt.bg=c(8,4), bg="white")
```
Complete Reindeer Pasture Data 
]
...
- Permutations can be constant or vary within clusters
...
- Partial ordination decomposes total variation $\Lambda$ into
  ...
  $\Lambda_k$ and into residuals $\Lambda_r$

- Sometimes we are interested in the relative magnitude of several
...
- $R^2$ is biased: even random data have $R^2>0$, and when you add
  ...
  that has expected value $R^2 = 0$ in random data
