...
- Use the linear combination of constraints as the explanatory
  variable, and find the linear combination that has the smallest
...
- The sum of first two unconstrained eigenvalues was
...
- LC scores really are linear combinations of constraints: and this
...
- Permutations can be constant or variable within clusters
...
.pull-left[
.small[
```{r bcifree, echo=FALSE, fig.height=3}
palette(viridis(8))
par(mar=c(4,4,0,1)+.1)
trellis.par.set("strip.background", lstrip)
...
.pull-right[
.small[
```{r bcigrid, echo=FALSE, fig.height=3}
palette(viridis(8))
par(mar=c(4,4,0,1)+.1)
trellis.par.set("strip.background", lstrip)
...
- If we have a strong prior theory, we keep and analyse all variabels
  of interest &mdash; even those that prove to be insignficant

- In explanatory analysis we choose the variables among candidates,
...
- In particular when explanatory variables are correlated, there may
  ...
  variables can have cascading effects
...
- The terms partialled out are called conditions
...
- The conditions can be background variables or confounding factors
  ...
  - Use *Plot* as a partialled out factor in split-plot designs

  - Remove uninteresting environmental variation before focusing on
...
- The interesting variable is the insecticide dose $\times$ time
...
- Time goes only forward and simultaneously in all plots, and
...
.small[
```{r echo=FALSE, dependson='setpyrifos'}
...
mod <- rda(pyrifos)
plot(mod, dis="si", scaling="si", type="n")
points(mod, dis="si", scaling="si", pch=16, cex=0.8, col=dose)
ordiarrows(mod, ditch, col=dose, scaling="si", lwd=2)
legend("topright", levels(dose), pch=16, lty=1, lwd=2, col=1:5, title="Pyrifos Dose") 
```
